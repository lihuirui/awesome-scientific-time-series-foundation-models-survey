...
\begin{itemize}
    ...
    \item \textbf{Probabilistic Metrics}: Computes continuous ranked probability scores (CRPS), spread-error ratios, and brier scores for ensemble predictions (demonstrated in GenCast~\cite{price2023gencast} and SEEDS~\cite{li2024seeds}).
...
El Aouni et al.~\cite{elaouni2025oceanbench} established \textbf{OceanBench} to evaluate data-driven global ocean forecasting systems against Copernicus Marine GLORYS12 reanalyses and satellite observations. Benchmarking sea surface height, temperature, and velocities across 1--10 day horizons, OceanBench provides the first unified benchmark assessing kinetic energy conservation and geostrophic balance in AI ocean models.

\subsubsection{SeisBench: Unified Benchmarking for Seismology}
In geophysics, Woollam et al.~\cite{woollam2022seisbench} established \textbf{SeisBench}, unifying five major international waveform catalogues into a standardized benchmark for machine learning in seismology. Benchmarking model architectures across phase picking, earthquake detection, and hypocenter localization, SeisBench systematically evaluates cross-network transferability and generalization across disparate tectonic regimes.

\subsubsection{Cross-Disciplinary Benchmarks: UniTS and SciTS}
Gao et al.~\cite{gao2024units} introduced a standardized 38-dataset benchmark across diverse physical, biomedical, and engineering domains, establishing unified evaluation protocols for multi-task time series models. Simultaneously, Wu et al.~\cite{wu2025scits} formulated the \textbf{SciTS} benchmark, spanning 12 scientific disciplines across 43 specialized tasks to evaluate both continuous numerical accuracy and multi-step reasoning capabilities in scientific foundation models.
